...
class ImportResource extends Resource
{
    ...
    {
        return $table->modifyQueryUsing(fn ($query) => $query->with('importSource.company'))->columns([TextColumn::make('importSource.name')->label('Bron'), TextColumn::make('importSource.company.name')->label('Bedrijf'), TextColumn::make('status')->badge(), TextColumn::make('total_rows')->label('Totaal'), TextColumn::make('imported_rows')->label('Nieuw'), TextColumn::make('updated_rows')->label('Bijgewerkt'), TextColumn::make('skipped_rows')->label('Overgeslagen'), TextColumn::make('failed_rows')->label('Mislukt'), TextColumn::make('started_at')->dateTime(), TextColumn::make('finished_at')->dateTime()])->recordActions([ViewAction::make()]);
    }

    public static function infolist(Schema $schema): Schema
    {
        return $schema->components([Section::make('Samenvatting')->schema([TextEntry::make('importSource.name')->label('Bron'), TextEntry::make('importSource.company.name')->label('Bedrijf'), TextEntry::make('status')->badge(), TextEntry::make('total_rows')->label('Totaal'), TextEntry::make('imported_rows')->label('Nieuw'), TextEntry::make('updated_rows')->label('Bijgewerkt'), TextEntry::make('skipped_rows')->label('Overgeslagen'), TextEntry::make('failed_rows')->label('Mislukt'), TextEntry::make('started_at')->dateTime(), TextEntry::make('finished_at')->dateTime()]), Section::make('Veilige logregels')->schema([RepeatableEntry::make('importLogs')->schema([TextEntry::make('level')->badge(), TextEntry::make('message'), TextEntry::make('context.position')->label('Recordpositie'), TextEntry::make('context.source_reference')->label('Bronreferentie'), TextEntry::make('context.code')->label('Uitkomstcode')])])]);
    }
